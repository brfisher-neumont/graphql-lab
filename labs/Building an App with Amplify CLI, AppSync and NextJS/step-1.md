...
3. What changes from the earlier labs: instead of an Express server you run and manage yourself, the GraphQL API is now hosted by **AWS AppSync**, its infrastructure (API, auth, resolvers) is defined and deployed with the **Amplify CLI**, and real sign-up/sign-in is handled by **Amazon Cognito** rather than by hand.
